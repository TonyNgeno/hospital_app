<?php 
	...
	if(isset($_POST['register'])){
		...
		if($password === $cPassword){
			$query = "INSERT INTO doctors(name, sex, age, profession ,username, password) VALUES(?,?,?,?,?,?)";
		
			$statement = $db->_conn->prepare($query);
			
			$name = strtoupper($name);
			
			$data = array($name, $sex, $age, $profession, $username, $password);
			...
			try{
				...
				if($statement->rowCount()){
					...
					redirectTo('doctorHome.php');
					
				}else{
					$msg = "Doctor not added, please try again";
				}
		...
		<div class = "col-lg-6 col-lg-offset-3">
			...
			<form class = "" action = "" method = "POST">
				<div class = "form-group">
					...
					<input type = "text" id = "name" placeholder = "doctor name" name = "name" class = "form-control text-uppercase" required />
				</div>
				...
				<div class = "form-group">
					...
					<input type = "number" max = "140" min = "18" id = "age" placeholder = "age" name = "age" class = "form-control" required />
				</div>
				
				<div class = "form-group">
					<label for = "profession">PROFESSION</label>
					<textarea id = "profession" placeholder = "profession" name = "profession" class = "form-control" required></textarea>
				</div>
